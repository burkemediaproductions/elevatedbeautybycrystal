...
function inferCategory(service){
  ...
  if(originalKey && originalKey !== 'services' && originalKey !== 'service'){
    return {key:slugify(originalKey), name:originalName || originalKey, url:`/services.html?category=${slugify(originalKey)}`, summary:`Browse ${originalName || originalKey} services from Crystal's Square Service Library.`};
  }
  ...
  return {key:'services', name:originalName && originalName !== 'Services' ? originalName : 'More Services', url:'/services.html?category=services', summary:'Additional services from Crystal\'s Square Service Library.'};
}
function normalizeSquareData(data){
...
function serviceCard(s,defaultUrl){const price=s.price?`<span>${escapeHtml(s.price)}</span>`:'';const duration=s.duration?`<span>${escapeHtml(s.duration)}</span>`:'';const url=s.bookingUrl||defaultUrl||'/booking.html';return `<article class="service-item"><div class="service-item-top"><div><h3>${escapeHtml(s.name)}</h3><p class="service-meta">${[escapeHtml(s.categoryName),duration,price].filter(Boolean).join(' • ')}</p></div></div><p>${escapeHtml(s.description||'Review this service and continue to Square to book.')}</p><a class="btn btn-primary" href="${escapeHtml(url)}" target="_blank" rel="noopener">Book / Pay on Square</a></article>`}
function categoryCard(c,bookingUrl){return `<article class="card service-card reveal visible"><span class="eyebrow">${escapeHtml(c.name)}</span><h3>${escapeHtml(c.name)}</h3><p>${escapeHtml(c.summary || `Browse ${c.name} services from Crystal's Square Service Library.`)}</p><div class="btn-row"><a class="btn btn-secondary" href="${escapeHtml(c.url || `/services.html?category=${c.key}`)}">View Services</a><a class="btn btn-primary" href="${escapeHtml(bookingUrl||'/booking.html')}" target="_blank" rel="noopener">Book on Square</a></div></article>`}
async function loadSquare(){try{const res=await fetch('/.netlify/functions/square-catalog',{headers:{'accept':'application/json'}});if(!res.ok)throw new Error('Square unavailable');return normalizeSquareData(await res.json())}catch(e){return normalizeSquareData({services:[],categories:EBC_FALLBACK_CATEGORIES,bookingSiteUrl:'/booking.html',status:'fallback'})}}
function selectedCategoryFromUrl(){return new URLSearchParams(window.location.search).get('category') || ''}
function renderServiceLists(data){const defaultUrl=data.bookingSiteUrl||'/booking.html';document.querySelectorAll('.square-service-list').forEach(el=>{let key=el.dataset.category||'all';const queryKey=selectedCategoryFromUrl();if(key==='all' && queryKey) key=queryKey;let services=(data.services||[]).filter(s=>key==='all'||s.categoryKey===key||s.rawCategoryKey===key||s.categoryName===key);if(!services.length){try{const fb=JSON.parse(el.dataset.fallback||'[]');services=fb.length?fb.map(name=>({name,categoryKey:key,categoryName:(EBC_CATEGORY_BY_KEY[key]||{}).name||'Service',description:'This fallback item will be replaced by the Square Service Library once configured.'})):fallbackServices(key)}catch(e){services=fallbackServices(key)}}const cat=(data.categories||[]).find(c=>c.key===key)||EBC_CATEGORY_BY_KEY[key];const heading=el.closest('section')?.querySelector('h2');if(queryKey && cat && heading) heading.textContent=`${cat.name} services`;el.innerHTML=services.map(s=>serviceCard(s,defaultUrl)).join('')})}
...
loadSquare().then(data=>{window.EBC_SQUARE_DATA=data;renderDynamicServiceNav(data);renderServiceLists(data);renderCategoryCards(data)});
