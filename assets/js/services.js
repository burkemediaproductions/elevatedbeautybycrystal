...
function inferCategory(service){
  ...
  if(originalKey && originalKey !== 'services' && originalKey !== 'service'){
    return {key:slugify(originalKey), name:originalName || originalKey, url:`/services.html?category=${slugify(originalKey)}`, summary:`Browse ${originalName || originalKey} services from Crystal's current service menu.`};
  }
  ...
  return {key:'services', name:originalName && originalName !== 'Services' ? originalName : 'More Services', url:'/services.html?category=services', summary:'Additional services from Crystal\'s current service menu.'};
}
