...
exports.handler = async function () {
  ...
  if (!token) return { statusCode: 200, headers, body: JSON.stringify({ status: 'missing_token', services: [], categories: [], bookingSiteUrl, categoryBookingLinks }) };
  try {
    ...
    if (!resp.ok) {
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ status: 'square_error', error: err, services: [], categories: [], bookingSiteUrl, categoryBookingLinks }) };
    }
    ...
    for (const item of json.items || []) {
      ...
      for (const vid of d.variations || []) {
        ...
        if (!v || !v.item_variation_data) continue;
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ status: 'ok', bookingSiteUrl, categoryBookingLinks, categories: Object.values(cats), services }) };
  } catch (error) {
    return { statusCode: 200, headers, body: JSON.stringify({ status: 'exception', error: error.message, services: [], categories: [], bookingSiteUrl, categoryBookingLinks }) };
  }
};
